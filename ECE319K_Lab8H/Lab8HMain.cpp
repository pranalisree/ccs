...
SlidePot Sensor(1847,138);  // copy calibration from Lab 7
...
void TIMG12_IRQHandler(void){
  ...
  if((TIMG12->CPU_INT.IIDX) == 1){  // acknowledge interrupt
    ...
    GPIOB->DOUTTGL31_0 = GREEN;   // heartbeat 1

    TransmitCount++;

    Sensor.Save(Sensor.In());     // sample ADC

    GPIOB->DOUTTGL31_0 = GREEN;   // heartbeat 2

    pos = Sensor.Distance();      // 0–2000
    uint32_t scaled = pos / 10;   // now 0–200

    // send framed message: <ABC
    IRxmt_OutChar('<');
    IRxmt_OutChar((scaled/100)%10 + '0'); // A
    IRxmt_OutChar((scaled/10)%10  + '0'); // B
    IRxmt_OutChar((scaled)%10     + '0'); // C

    GPIOB->DOUTTGL31_0 = GREEN;   // heartbeat 3
  }
}
// Connect dual channel scope to transmitter sensor PA8, sensor output, receiver PA22
...
// toggle a PB27 heartbeat (change from 0 to 1, or from 1 to 0), 
// sample the ADC (with or without hardware averaging),
// toggle a PB27 heartbeat (change from 0 to 1, or from 1 to 0), 
// convert to distance and create the 4-byte message
// send the 4-byte message to the other computer (calls IRxmt_OutChar 4 times)
...
int main(void){ // main5T
  ...
  Sensor.Init();              // PB18 = ADC1 channel 5
  IRxmt_Init();               // PA8 transmit

  TimerG12_IntArm(2666667, 2); // 30 Hz

  __enable_irq();

  while(1){
    // all work happens in interrupt
  }
}
