...
SlidePot Sensor(1847,138); // needs calibration
...
void TIMG12_IRQHandler(void){
  if((TIMG12->CPU_INT.IIDX) == 1){ // this will acknowledge
    ...
    // sample
    Data = Sensor.In();
    Sensor.Save(Data);  // store data into mailbox
    GPIOB->DOUTTGL31_0 = GREEN; // toggle PB27 (minimally intrusive debugging)
  }
}
...
uint32_t PlotTime;
  ...
  TimerG12_IntArm(39999, 1); //make it 2 KHLZ
  ...
  while(1){
    ...
    Sensor.Sync();
   // GPIOB->DOUTTGL31_0 = RED;  // toggle red LED2 on Port B, PB26
   Position = Sensor.Distance();
   // move cursor to top
   ST7735_SetCursor(0,0);
   // display distance in top row OutFix
    startTime = SysTick->VAL;
    OutFix(Position);// convert Data to Position );
    stopTime = SysTick->VAL;
    OutFixtime = ((startTime-stopTime)&0x0FFFFFF)-Offset; // in bus cycles
    ...
    if((Time%15)==0){
      ...
      stopTime = SysTick->VAL;
      PlotTime = ((startTime-stopTime)&0x0FFFFFF)-Offset; // in bus cycles
